...
export const defineMonacoThemes = (monaco: Monaco) => {
  Object.entries(THEME_DEFINITONS).forEach(([themeName, themeData]) => {
    // Restrict to Monaco built-in themes
    const validThemes: MonacoBaseTheme[] = ['vs', 'vs-dark', 'hc-black'];

    if (validThemes.includes(themeData.base as MonacoBaseTheme)) {
      monaco.editor.defineTheme(themeName, {
        base: themeData.base as MonacoBaseTheme, // Ensuring the base is a valid Monaco built-in theme
        inherit: themeData.inherit,
        rules: themeData.rules.map((rule) => ({
          ...rule,
          foreground: rule.foreground, // Ensuring foreground is correctly typed
        })),
        colors: themeData.colors, // Ensure color definitions are passed
      });
    } else {
      console.error(`Invalid base theme: ${themeData.base}. It must be one of 'vs', 'vs-dark', or 'hc-black'.`);
    }
  });
};